...
const offTopicResponse = (library) => {
  const librarySuffix = library?.name ? ` (${library.name})` : '';
  return {
    message: `Maaf, saya hanya bisa membantu seputar instalasi dan konfigurasi graphics library${librarySuffix}.
...
- 🎮 GPU driver & graphics troubleshooting`,
  ...
  const extractConversationPreferences = (chatHistory = []) => {
    const text = Array.isArray(chatHistory)
      ? chatHistory.map((entry) => entry.content || '').join(' ').toLowerCase()
      : '';
    const preferences = [];
    if (text.includes('chocolatey')) preferences.push('Prefer package manager: Chocolatey');
    if (/jangan (pakai )?vcpkg|bukan vcpkg|tidak pakai vcpkg|tanpa vcpkg/.test(text)) {
      preferences.push('Hindari penggunaan vcpkg karena diminta oleh user.');
    }
    return preferences;
  };
...
Jika user sebelumnya menyatakan pilihan package manager tertentu (misalnya Chocolatey vs vcpkg), gunakan pilihan itu sebagai preferensi utama.
...
Jika pertanyaan di luar topik, tolak dengan sopan.`;
  ...
  const generateAIResponse = async (message, context = {}) => {
    ...
    const preferenceLines = extractConversationPreferences(chatHistory);
    const preferencePrompt = preferenceLines.length > 0
      ? `Penting: ${preferenceLines.join(' ')}\n\n`
      : '';
      ...
      try {
        ...
        const completion = await groq.chat.completions.create({
          ...
          messages: [
            ...
            ...(
              preferencePrompt
                ? [{ role: 'assistant', content: preferencePrompt }]
                : []
            ),
